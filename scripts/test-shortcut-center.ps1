...
if ([string]::IsNullOrWhiteSpace($ResultsDirectory)) {
    ...
    $ResultsDirectory = Join-Path $temporaryRoot 'mypowertools-shortcut-results'
}
...
& dotnet test (Join-Path $repo 'tests/ShortcutCenter.Tests/ShortcutCenter.Tests.csproj') --configuration $Configuration --logger 'trx;LogFileName=shortcuts.trx' --results-directory $ResultsDirectory -p:BuildWindowsWebToolHost=false
if ($LASTEXITCODE -ne 0) { throw 'Shortcut center regression tests failed.' }
...
if ($LASTEXITCODE -ne 0) { throw 'Existing personal UX regression tests failed.' }
