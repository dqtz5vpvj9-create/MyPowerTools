...
- **macOS**：从 [最新发布](https://github.com/dqtz5vpvj9-create/MyPowerTools/releases/latest) 下载 `MyPowerTools-Installer-macos-arm64.zip`（Apple 芯片）或 `MyPowerTools-Installer-macos-x64.zip`（Intel），解压后双击「MyPowerTools Installer」即可安装和更新，无需终端。首次打开的放行步骤见 [macOS 安装器](docs/MACOS_INSTALLER.md)。

## 个人开发主线

统一使用 **`main`**；旧功能分支已归档。macOS 构建、子模块更新和恢复说明见 [单一主线指南](docs/SINGLE_MAIN.md)。
