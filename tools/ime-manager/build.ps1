...
$sdkToolProject = Join-Path $sdkToolRoot 'src\ImeManager.Tool\ImeManager.Tool.csproj'
$runtimeProject = Join-Path $sdkToolRoot 'src\ImeManager.Runtime\ImeManager.Runtime.csproj'
...
$artifactPackage = Join-Path $artifactsRoot 'ime-manager.mptpkg'
...
$expectedSdkTool = Join-Path $sdkToolRoot "src\ImeManager.Tool\bin\$Configuration\net10.0\ImeManager.Tool.dll"
$expectedRuntime = Join-Path $sdkToolRoot "src\ImeManager.Runtime\bin\$Configuration\net10.0\ImeManager.Runtime.exe"
foreach ($expectedPath in @($expectedSdkTool, $expectedRuntime, $artifactPackage)) {
...
# tool.json uses stable Release-shaped paths. During a Debug developer overlay,
# replace those staged files with the configuration built by this invocation.
$configurationOutputs = @(
    [ordered]@{
        Source = Split-Path -Parent $expectedSdkTool
        Destination = Join-Path $artifactRuntime 'src\ImeManager.Tool\bin\Release\net10.0'
    },
    [ordered]@{
        Source = Split-Path -Parent $expectedRuntime
        Destination = Join-Path $artifactRuntime 'src\ImeManager.Runtime\bin\Release\net10.0'
    }
)
foreach ($configurationOutput in $configurationOutputs) {
    New-Item -ItemType Directory -Path $configurationOutput.Destination -Force | Out-Null
    Get-ChildItem -LiteralPath $configurationOutput.Source -File |
        Copy-Item -Destination $configurationOutput.Destination -Force
}
...
$moduleManifest = [ordered]@{
    ...
    runtimePolicy = [ordered]@{
        ...
        operationRules = [ordered]@{
            ...
            systemMutation = 'sidecar-required'
