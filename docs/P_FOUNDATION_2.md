...
- Reduced `MainWindow.cs` to a 78-line bootstrap wrapper for Shell chrome construction, named host lookup, startup options, lifecycle hooks, and keyboard dispatch.
...
- `src/MyPowerTools.Shell.Avalonia/MainWindow.cs` current: 78 lines.
...
| MainWindow size | target <= 250 lines | current: 78 lines |
