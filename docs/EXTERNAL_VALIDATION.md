...
| macOS native validation | macOS platform project compiles with degraded hotkey, privilege, notification, autostart, service, network, display, tray, and secret providers. | macOS host with .NET SDK 10. | Build, module validation, Runner once, Shell smoke, UDS transport, and degraded providers behave as documented. |
