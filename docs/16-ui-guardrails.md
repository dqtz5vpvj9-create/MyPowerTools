...
`mpt ui snapshot` 生成标准 contract JSON 和 deterministic PNG pixel snapshot。`mpt ui check` 执行 surface schema、状态覆盖和组件白名单门禁；baseline diff 使用 snapshot manifest 中的 `sourceSha256`、`pixelSha256`、尺寸、颜色数和非背景像素指标。
