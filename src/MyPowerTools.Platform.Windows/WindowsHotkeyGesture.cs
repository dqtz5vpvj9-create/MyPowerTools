...
public sealed record WindowsHotkeyGesture(uint Modifiers, uint VirtualKey, string NormalizedGesture)
{
    ...
    public static bool TryParse(string gesture, out WindowsHotkeyGesture? parsed, out string error)
    {
        ...
        {
            error = "Hotkey gesture is required.";
            return false;
        }
        ...
        foreach (var token in tokens)
        {
            ...
            {
                error = $"Hotkey gesture '{gesture}' contains more than one key.";
                return false;
            }
        ...
        if (modifiers == 0)
        {
            error = "Global hotkeys require at least one modifier.";
            return false;
        }

        if (keyToken is null)
        {
            error = "Hotkey gesture must include a key.";
            return false;
        }

        if (!TryParseKey(keyToken, out var virtualKey, out var normalizedKey))
        {
            error = $"Hotkey key '{keyToken}' is unsupported.";
            return false;
        }

        parsed = new WindowsHotkeyGesture(modifiers, virtualKey, NormalizeGesture(modifiers, normalizedKey));
        return true;
    }
    ...
    private static string NormalizeGesture(uint modifiers, string key)
    {
